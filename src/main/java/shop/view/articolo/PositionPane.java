...
public class PositionPane extends JFrame implements ActionListener {
    ...
    public PositionPane(String attribute, List<String> elements) {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == btn_add) {
            ...
            DefaultComboBoxModel<String> model = new DefaultComboBoxModel<>(loadProductAttribute(Attribute.POSIZIONE.getAttribute()).toArray(new String[0]));
            ...
            while (table.getSelectedRow() >= 0) {
                try {
                    Connection con = (new ConnectionManager()).getConnection();
                    Statement stmt = con.createStatement();
                    stmt.executeUpdate(String.format("DELETE FROM POSIZIONE_PRODOTTO WHERE POSIZIONE='%s'", table.getValueAt(table.getSelectedRow(), 0)));
                    stmt.executeUpdate(String.format("UPDATE ARTICOLO SET POSIZIONE='%s' WHERE POSIZIONE='%s'", "NOT CATEGORIZED", table.getValueAt(table.getSelectedRow(), 0)));
                    stmt.close();
                    con.close();
                } catch (Exception ex) {
                    System.out.println(ex.getMessage());
            ...
            loadArticleFromDB().forEach(article -> ArticoloPane.tableModel.addRow(new String[]{article.getCodice(), article.getDescrizione(), article.getCategoria(), article.getPosizione(), article.getUnita(), String.valueOf(article.getPrezzo()).replace(".", ",").concat(" €"), String.valueOf(article.getScorta()), article.getProvenienza()}));
